...
def build_ydl_opts(extra: Optional[dict] = None) -> dict:
    ...
    # 読み取り専用FS（Replit /etc/secrets など）の場合は /tmp にコピーして使用
    cookie_path: Optional[str] = None
    for candidate in filter(None, [COOKIES_FILE, "/tmp/cookies.txt"]):
        resolved = _resolve_cookie_path(candidate)
        if resolved:
...
def _resolve_cookie_path(path: str) -> Optional[str]:
    """
    クッキーファイルのパスを解決する。

    /etc/secrets/ など読み取り専用FSにある場合（Replit 等）、
    yt-dlp がクッキーを更新しようとして EROFS エラーになるため、
    書き込み可能な /tmp/ にコピーしてそのパスを返す。
    ...
    # 親ディレクトリが書き込み可能 → そのまま返す
    if os.access(parent_dir, os.W_OK):
        return path

    # 読み取り専用 → /tmp にコピーして書き込み可能なパスを返す
    safe_name = hashlib.md5(path.encode()).hexdigest()[:12]
    tmp_path = f"/tmp/yt_cookies_{safe_name}.txt"
    try:
        shutil.copy2(path, tmp_path)
        os.chmod(tmp_path, 0o600)   # 他ユーザーから保護
        logger.info("Copied cookies to writable path: %s → %s", path, tmp_path)
        return tmp_path
    except Exception as e:
        logger.warning("Failed to copy cookies file: %s", e)
        return None
